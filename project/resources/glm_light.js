/*
* QUESTA è una libreria che serve a leggere e caricare dei file obj per importare delle mesh.
* Tuttavia è molto limitata, permette di caricare solo alcun informazioni come vertici, edge e facce, e solo mesh con faccette quadrilatere (e triangolari).
* Fa parte del progetto d'esame estendere questa libreria per far importare anche ulteriori informazioni come i colori, le normali ecc...
*/
...
//        model.normals[3 * i + 2] -= cz;
	...
	*/
	...
	function SecondPass(model, data, mesh) {
	  ...
	  /*EDITED BY ME*/
	  mesh.numnorm = model.numnormals;
	  mesh.numtext = model.numtexcoords;

	    /* on the second pass through the file, read all the data into the
	    allocated arrays */
    //numvertices = numnormals = numtexcoords = 1;
	    numvertices = 1;		//Gli indici che andrò a leggere nell'obj quando ho -f partono da 1
	    numnormals = 1;
 	    numtexcoords = 1;
	    numtriangles = 0;
	    material = 0;
	...
	/*EDITED BY ME*/
	  mesh.norm  = new Array(mesh.numnorm+1);
	  for (var h=0; h<mesh.numnorm+1; h++)
		  mesh.norm[h] = { x:0, y:0, z:0};
	  
	  mesh.textcoord = new Array(mesh.numtext+1);
	  	  for (var h=0; h<mesh.numtext+1; h++)
		  mesh.textcoord[h] = { u:0, v:0 };
		...
		case 'v':               /* v, vn, vt */
		 ...
		 //console.log (mesh.vert[numvertices]);
//         console.log(1,numvertices);
		...
		case 'vn':               /* v, vn, vt */
         ...
         mesh.norm[numnormals].z=parseFloat(buf[3]);
			numnormals++;
		//console.log (mesh.norm[numnormals]);
			break;
		case 'vt':               /* v, vn, vt */
			...
			texcoords[2 * numtexcoords + 1] = parseFloat(buf[2]);
				...
				/*EDITED BY ME*/
		 mesh.textcoord[numtexcoords].u=parseFloat(buf[1]);
         mesh.textcoord[numtexcoords].v=parseFloat(buf[2]);
			numtexcoords++;
			//console.log (mesh.textcoord[numtexcoords]);
...
//			model.triangles[numtriangles].nv = 4;//buf.length - 1;
			model.FindGroup(group.name).triangles[model.FindGroup(group.name).numtriangles++] = numtriangles;
	      	numtriangles++;
	      mesh.face[numtriangles].n_v_e=model.triangles[numtriangles-1].nv;
          for (var jj=0; jj<mesh.face[numtriangles].n_v_e; jj++)		//QUI CARICO dentro alla faccia i-esima l'indice dei suoi tre vertici, normali e coord texture.
	      {
	        mesh.face[numtriangles].vert[jj]=model.triangles[numtriangles-1].vindices[jj];		
			/*EDITED BY ME*/
			mesh.face[numtriangles].tex[jj]=model.triangles[numtriangles-1].tindices[jj];
 ...
 * -tex
 * -norm
 * contengono rispettivamente gli indici di vertice, di texture e della normale di ognuno dei vertici di quella faccia.
 *
 * Esempio:
 * mesh.face[i].vert[0] restituisce l'indice del vertice 0 della faccia i-esima.
 *
 * Gli indici possono essere sfruttati per risalire alle coordinate vere e proprie dei vertici, texture e normali, che sono salvati invece negli array:
 * -mesh.vert
 * -mesh.textcoord
 * -mesh.norm
 *
 * Esempio: per risalire alla coordinata x del vertice 0 della faccia i-esima:
